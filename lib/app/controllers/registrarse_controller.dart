...
class RegistrarseController extends GetxController {
  ...
  final RegisterProvider registerProvider =
      RegisterProvider(); // Instancia del proveedor
  ...
  Future<bool> verificarEmail(String email) async {
    try {
      ...
      EasyLoading.showInfo(
          'Error de conexion'.toString()); // Muestra el mensaje de error
  ...
  Future<void> validateCredentials() async {
    ...
    if (passwordController.text == passwordConfirmController.text) {
      ...
      if (numberVerificado && emailVerificado) {
        ...
        EasyLoading.showInfo('Numero y correo ya esta en uso'
            .toString()); // Muestra el mensaje de error
      } else if (emailVerificado) {
        isUserEmail.value = true;
        isNumberAlert.value = false;

        EasyLoading.showInfo(
            'Correo ya esta en uso'.toString()); // Muestra el mensaje de error
      } else if (numberVerificado) {
        isNumberAlert.value = true;
        isUserEmail.value = false;
        EasyLoading.showInfo(
            'Numero ya esta en uso'.toString()); // Muestra el mensaje de error
      } else {
        isUserEmail.value = false;
        isNumberAlert.value = false;
        ...
        EasyLoading.showInfo(
            'Datos Correctos'.toString()); // Muestra el mensaje de error
        clearLoginData();
      ...
      EasyLoading.showInfo(
          'Verifica contraseña'.toString()); // Muestra el mensaje de error
    }
  }
